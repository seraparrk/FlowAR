...
public class handrotate : MonoBehaviour
{
    ...
    /* 3d joint skeleton 값을 기반으로 손을 탐지해서, 손 위에 객체를 생성해줄 것이다. */

    // 3D joint skeleton -> 빌드 결과물에서 뽑아온 enum 클래스
    enum JointIndices
    {
        Invalid = -1,
    ...
    void OnHumanBodiedChanged(ARHumanBodiesChangedEventArgs eventArgs)
    {
        foreach (ARHumanBody humanBody in eventArgs.updated)
        {
            ...
            {
                if (joint.index == (int)JointIndices.LeftHandMidEnd || joint.index == (int)JointIndices.RightHandMidEnd)
                {
